...
export function ProductListPage() {
  ...
  const filter: CategoryFilter = searchParams.get("category_id") ?? "all";
  ...
  // Category filtering happens server-side now (the catalog can grow past
  // what's reasonable to filter client-side), so any filter change means a
  // fresh page-1 fetch rather than re-slicing an already-loaded array.
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    setError(null);

    fetchProducts(filter === "all" ? undefined : filter, 1, PAGE_SIZE)
      .then((data) => {
        if (cancelled) return;
  ...
  function handleLoadMore() {
    ...
    setLoadingMore(true);
    fetchProducts(filter === "all" ? undefined : filter, nextPage, PAGE_SIZE)
      .then((data) => {
